...
def importer(body):
    ...
    name = re.search('Target body name:(.*) \(', parse_json["result"]).group(1).replace(" ", "")
    ...
    pos = pos.replace("Z=", ",")
    pos = pos.split(",")
    pos = [float(i)*1000 for i in pos] # Horizons uses km and common sense uses meters so I convert
    ...
    vel = vel.replace("VZ=", ",")
    vel = vel.split(",")
    vel = [float(i)*1000 for i in vel] # Horizons uses km/s and common sense uses m/s so I convert
